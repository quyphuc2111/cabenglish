...
export function ProgressStats({ onOpen, t, courseData, classroomData, currentTheme = "theme-red" }: ProgressStatsProps) {
  ...
  const renderProgressItem = (item: ClassroomType) => {
    ...
    return (
      <motion.div
        ...
        className={`${getProgressBgColor(progress)} rounded-2xl p-4 sm:p-6 mb-4 shadow-xl hover:shadow-2xl transition-all duration-300 border border-white/80 backdrop-blur-sm relative overflow-hidden group`}
      >
        {/* Background pattern overlay */}
        <div className="absolute inset-0 opacity-5">
          <div className="absolute inset-0 bg-[url('/assets/bg-pattern.svg')] bg-repeat opacity-30"></div>
        </div>
        
        {/* Animated background glow */}
        <div className={`absolute -inset-2 bg-gradient-to-r ${getProgressColor(progress)} opacity-10 blur-xl rounded-3xl transition-all duration-500 group-hover:opacity-20`}></div>
        
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 relative z-10">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 bg-gradient-to-br ${currentColors.iconBg} rounded-full flex items-center justify-center shadow-lg border border-white/30 relative`}>
              {/* Icon glow effect */}
              <div className={`absolute -inset-1 bg-gradient-to-br ${currentColors.iconBg} rounded-full opacity-30 blur-sm`}></div>
              <OptimizeImage 
            ...
            <div>
              <p className="text-xs sm:text-sm font-medium text-gray-700 mb-1">Lớp học</p>
              <div className="bg-white/90 backdrop-blur-sm px-3 py-2 rounded-xl border border-white/80 shadow-md hover:shadow-lg transition-all duration-300">
                <p className="text-sm sm:text-base font-bold text-gray-800">{item.classname}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1">
            <div className="flex items-center gap-4">
              <div className="relative flex-1 group">
                {/* Progress background theo theme */}
                ...
                <div className="relative h-8 sm:h-10 rounded-full bg-white/60 backdrop-blur-sm border-2 border-white/80 cursor-pointer transition-all duration-500 hover:scale-[1.02] hover:border-white overflow-hidden"
                  ...
                  <div 
                    className={`absolute inset-0 bg-gradient-to-r ${getProgressColor(progress)} transition-all duration-1000 ease-out rounded-full`} 
                  ...
                  <div className="absolute inset-0 flex items-center justify-center z-30">
                    <span className={`font-bold text-sm sm:text-base transition-all duration-300 bg-gradient-to-r ${currentColors.iconBg} bg-clip-text text-transparent font-extrabold`}>
                      {progress}%
                    </span>
                  </div>
                </div>
                ...
                <div 
                  className={`absolute -inset-1 bg-gradient-to-r ${getProgressColor(progress)} rounded-full opacity-30 blur-sm transition-all duration-500 group-hover:opacity-50 z-0`}
              ...
              >
                <div className="relative">
                  <div className={`absolute inset-0 bg-gradient-to-br ${currentColors.iconBg} rounded-full blur opacity-60`}></div>
                  <div className="relative bg-white/20 backdrop-blur-sm rounded-full p-2 border border-white/40">
                    <Image
                      src="/reset_icon.png"
                      alt="reset"
                      width={40}
  ...
  return (
    <motion.div 
      ...
      className="w-full xl:w-1/2 pr-0 xl:pr-6 mb-6 xl:mb-0 relative"
    >
      {/* Header với theme-based gradient background */}
      <div className={`bg-gradient-to-r ${currentColors.headerGradient} rounded-2xl p-6 mb-6 shadow-xl`}>
        <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 justify-center text-white">
          <div className="relative">
            ...
            <div className="relative w-12 h-12 sm:w-14 sm:h-14 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center border border-white/30">
              <Image
                src="/percent.png"
                alt="percent"
                width={32}
      ...
      >
        <DialogContent className="w-[95vw] max-w-[900px] h-[90vh] max-h-[700px] p-0 border-0 bg-gradient-to-br from-slate-50 to-blue-50">
          <div className={`bg-gradient-to-r ${currentColors.modalHeader} p-6 rounded-t-lg`}>
            <DialogHeader>
