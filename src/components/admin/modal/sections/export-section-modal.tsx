...
function ExportSectionModal() {
  ...
  const handleExport = async () => {
    try {
      ...
      if (exportOption === "all" && sectionData?.data) {
        ...
        // Định nghĩa columns
        worksheet.columns = [
            { header: 'Tên phần', key: 'sectionName', width: 50 },
            { header: 'Thời gian ước tính', key: 'estimateTime', width: 20 },
            { header: 'Thứ tự', key: 'order', width: 20 },
          { header: 'Hình ảnh', key: 'iconUrl', width: 50 },
        ];

        // Thêm dữ liệu
        sectionData?.data.forEach((section: any) => {
          worksheet.addRow({
        ...
        a.href = url;
        a.download = 'danh-sach-section.xlsx';
        a.click();
  ...
  if (!isOpen || type !== "exportSection") return null;
        ...
        <DialogHeader>
          <DialogTitle>
            ...
            >
              ...
              <span className="text-xl font-medium">Xuất Dữ Liệu Section</span>
            </motion.div>
          </DialogTitle>
        </DialogHeader>
