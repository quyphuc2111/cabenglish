...
function ExportSchoolWeekModal() {
    const [isExporting, setIsExporting] = React.useState(false);
    const [exportOption, setExportOption] = React.useState<string>("all");

    const { isOpen, onClose, type } = useModal();
    const { data: schoolweeks, isLoading } = useSchoolWeek();
  const handleExport = async () => {
    try {
      setIsExporting(true);
      if (exportOption === "all" && schoolweeks) {
        // Tạo dữ liệu xuất với các trường được chọn
        const exportData = schoolweeks.data.map((schoolweek: SchoolWeekType) => ({
          'Tuần học': schoolweek.value,
        }));
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Danh sách tuần học');

        // Xuất file Excel
        XLSX.writeFile(wb, 'danh-sach-tuan-hoc.xlsx');
  ...
  if (!isOpen || type !== "exportSchoolWeek") return null;
        ...
        <DialogHeader>
          <DialogTitle>
            ...
            >
              ...
              <span className="text-xl font-medium">Xuất Dữ Liệu Lớp Học</span>
            </motion.div>
          </DialogTitle>
        </DialogHeader>
...
export default ExportSchoolWeekModal;
