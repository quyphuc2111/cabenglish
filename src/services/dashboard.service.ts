...
import { initializeProgress, resetLessonProgress } from "@/actions/progressAction";
import { getAllClassroomDataByUserId } from "@/actions/classroomAction";

export const DashboardService = {
  async fetchDashboardData(userId: string) {
    try {
      const [courseData, filterData, lockedData, classroomData, progressData] = await Promise.all([
